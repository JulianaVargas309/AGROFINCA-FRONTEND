    ...
    <div className="space-y-6">
      ...
      {gasto && (
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card><div className="flex items-center gap-2 text-stone-500 mb-1"><DollarSign size={16} /><span className="text-xs font-medium">Monto</span></div><p className="text-2xl font-bold text-red-600">{formatCurrency(gasto.monto)}</p></Card>
            {gasto.categoria && <Card><div className="text-xs text-stone-500 mb-1">Categoría</div><Badge>{CATEGORIA_GASTO_OPTIONS.find((c) => c.value === gasto.categoria)?.label || gasto.categoria}</Badge></Card>}
            {gasto.lote && <Card><div className="flex items-center gap-2 text-stone-500 mb-1"><MapPin size={16} /><span className="text-xs font-medium">Lote</span></div><p className="text-sm font-semibold">{gasto.lote.nombre}</p></Card>}
            {gasto.cultivo && <Card><div className="flex items-center gap-2 text-stone-500 mb-1"><Sprout size={16} /><span className="text-xs font-medium">Cultivo</span></div><p className="text-sm font-semibold">{gasto.cultivo.nombre}</p></Card>}
          </div>
        </>
      )}
      ...
      {venta && (
        <>
          ...
          {error && <Alert severity="error">{error}</Alert>}
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card><div className="flex items-center gap-2 text-stone-500 mb-1"><DollarSign size={16} /><span className="text-xs font-medium">Total</span></div><p className="text-2xl font-bold text-emerald-700">{formatCurrency(venta.total)}</p></Card>
            <Card><div className="flex items-center gap-2 text-stone-500 mb-1"><ShoppingCart size={16} /><span className="text-xs font-medium">Estado</span></div><Badge color={venta.estado === "COMPLETADA" ? "success" : venta.estado === "PENDIENTE" ? "warning" : "error"}>{venta.estado}</Badge></Card>
            {venta.cliente && <Card><div className="flex items-center gap-2 text-stone-500 mb-1"><User size={16} /><span className="text-xs font-medium">Cliente</span></div><p className="text-sm font-semibold">{venta.cliente.nombre}</p></Card>}
          </div>
          {venta.detalles && venta.detalles.length > 0 && (
            <Card>
              <h3 className="text-sm font-semibold text-stone-700 mb-3">Detalles</h3>
              <div className="divide-y divide-stone-100 -mx-5">
                {venta.detalles.map((d, i) => (
                  <div key={i} className="flex items-center justify-between px-5 py-3">
                    <div><p className="text-sm text-stone-700">{d.producto?.nombre || `Producto #${d.productoId}`}</p><p className="text-xs text-stone-500">{d.cantidad} x {formatCurrency(d.precioUnitario)}</p></div>
                    <p className="text-sm font-bold text-stone-700">{formatCurrency(d.subtotal)}</p>
                  </div>
                ))}
      ...
      {editModal.isOpen && gasto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 mx-4 w-full max-w-lg rounded-xl bg-white p-6 shadow-xl max-h-[90vh] overflow-auto">
            <h2 className="text-lg font-semibold text-stone-900 mb-4">Editar Gasto</h2>
