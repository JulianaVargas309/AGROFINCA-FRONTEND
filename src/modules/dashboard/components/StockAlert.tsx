  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-semibold text-stone-700 dark:text-stone-300">Alertas de Stock Bajo</h3>
        {productos.length > 0 && (
      ...
      {productos.length === 0 ? (
        <div className="py-4 text-center">
          ...
          <p className="mt-2 text-xs text-stone-500 dark:text-stone-400">Todo el inventario tiene stock suficiente.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {productos.map((p) => (
            <div key={p.id} className="flex items-center justify-between rounded-lg bg-red-50 px-3 py-2 dark:bg-red-900/20">
              <div>
                <p className="text-sm font-medium text-red-800 dark:text-red-300">{p.nombre}</p>
